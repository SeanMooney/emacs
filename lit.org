...
#+BEGIN_SRC emacs-lisp
  ...
  ;; Transient menu framework (used by magit, gptel, and other packages)
    ...
    (with-current-buffer buf
      (pcase my/ai-term-backend
        ('ghostel
         (when (and (boundp 'ghostel--process) ghostel--process
                    (process-live-p ghostel--process))
           (process-send-string ghostel--process (concat string "\n"))))
        ('eat
...
#+END_SRC
** Vault Sync
Automatically commit and push every saved note to the remote git repository, keeping the vault
in sync across machines. The hook activates ~git-auto-commit-mode~ on any file visited inside
~org-roam-directory~, so no ~.dir-locals.el~ is needed in the vault itself. The package is
skipped entirely when the vault directory is absent.
