...
#+BEGIN_SRC emacs-lisp
  ...
  (defun my/ai-term--ensure-ghostel-module ()
    "Ensure the ghostel native module is loaded, prompting to install if needed."
      ...
      (let ((dir (file-name-directory (locate-library "ghostel"))))
        ...
        (let ((mod (expand-file-name
          ...
          (unless (file-exists-p mod)
            (user-error "Ghostel native module not available; run M-x ghostel-download-module"))
          (module-load mod)))))
    ...
    (let ((buf (get-buffer-create buf-name)))
      (with-current-buffer buf
        (let ((default-directory dir)
          ...
          ;; Start with default dimensions; the window-size hook will resize
          ;; correctly once the buffer is displayed in a side window.
          (setq ghostel--term (ghostel--new 24 80 ghostel-max-scrollback))
          ...
          (when (buffer-live-p b)
            (with-current-buffer b
              (when (and (boundp 'ghostel--process) ghostel--process
                         (process-live-p ghostel--process))
                (process-send-string ghostel--process (concat p "\n"))))))
        buf program)
      buf))

  (defun my/ai-term--make-eat (buf-name dir program)
    "Create an eat terminal buffer BUF-NAME in DIR running PROGRAM directly."
    (let ((default-directory dir))
      (eat-make (string-trim buf-name "*") program)))

  (defun my/ai-term--make-ansi-term (buf-name dir program)
    "Create an ansi-term buffer BUF-NAME in DIR running PROGRAM."
    (let ((default-directory dir))
      (with-current-buffer (ansi-term program (string-trim buf-name "*"))
        (current-buffer))))
  ...
  (defun my/pi-no-sandbox ()
    ...
    (my/ai-term-launch "pi-no-sandbox"
                       (concat (or (executable-find "pi") "pi") " --no-sandbox")))
