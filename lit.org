...
Ghostel note: ghostel always starts a shell rather than an arbitrary program directly.
The launcher creates a ghostel shell buffer in the project directory, then sends the
program command to it after a short delay.
...
#+BEGIN_SRC emacs-lisp
  ...
  (defcustom my/ai-term-ghostel-delay 0.3
    "Seconds to wait after ghostel shell starts before sending the AI program command."
    :type 'number
    :group 'tools)
  ...
  (defun my/ai-term--project-name (dir)
    "Return the short project name for DIR (last path component)."
    (file-name-nondirectory (directory-file-name (file-truename dir))))

  (defun my/ai-term--buffer-name (tool dir)
    "Return the buffer name for TOOL running in DIR: *<project>-<tool>*."
    (format "*%s-%s*" (my/ai-term--project-name dir) tool))

  (defun my/ai-term--buffer-alive-p (buf)
    "Return non-nil if BUF has a live terminal process."
    (when (buffer-live-p buf)
      (with-current-buffer buf
        (pcase my/ai-term-backend
          ('ghostel   (and (boundp 'ghostel--process) ghostel--process
                           (process-live-p ghostel--process)))
          ('eat       (and (boundp 'eat-terminal) eat-terminal
                           (eat-term-live-p eat-terminal)))
          ('ansi-term (get-buffer-process buf))))))
  ...
  (defun my/ai-term--find-project-buffer (dir)
    "Find any live AI terminal buffer for DIR, regardless of tool.
  Matches buffers named *<project>-<tool>* where project is derived from DIR."
    (let ((project-name (my/ai-term--project-name dir)))
      (seq-find
       (lambda (b)
         (string-match-p
  ...
  (defun my/ai-term--ensure-ghostel-module ()
    "Ensure ghostel and its native module are loaded."
    (require 'ghostel)
    ;; Newer ghostel exposes `ghostel--load-module'; older versions used
    ;; `ghostel--ensure-module'.  Prefer the current entry point because it
    ;; handles downloading/building the sidecar module correctly.
    (cond
     ((fboundp 'ghostel--load-module)
      (ghostel--load-module t))
     ((fboundp 'ghostel--ensure-module)
      (let ((dir (file-name-directory (locate-library "ghostel"))))
        (ghostel--ensure-module dir)))
     ((not (fboundp 'ghostel--new))
      (user-error "Ghostel native module loader is not available; try M-x ghostel"))))

  (defun my/ai-term--make-ghostel (buf-name dir program)
    "Create a fresh ghostel terminal buffer BUF-NAME in DIR, then send PROGRAM.
  Initialises the ghostel buffer directly (without calling `ghostel') so that
  the existing project ghostel terminal window is not disturbed."
    (my/ai-term--ensure-ghostel-module)
    ;; Caller has already killed any stale buffer; create fresh.
    (let ((buf (get-buffer-create buf-name)))
      (with-current-buffer buf
        (let ((default-directory dir)
              (ghostel-kill-buffer-on-exit nil))
          ;; Use ghostel's own buffer initialisation helper instead of
          ;; constructing `ghostel--term' by hand.  The internal API changed in
          ;; newer ghostel versions, and this keeps the launcher aligned with
          ;; ghostel's required buffer-local state.
          (ghostel--init-buffer buf 24 80)
          (setq ghostel--managed-buffer-name (buffer-name))
          (setq ghostel--buffer-identity (buffer-name))
          (ghostel--start-process)))
      ;; Send the AI program name to the shell after it is ready.
      (run-with-timer my/ai-term-ghostel-delay nil
        (lambda (b p)
          (when (buffer-live-p b)
            (with-current-buffer b
              (when (and (boundp 'ghostel--process) ghostel--process
                         (process-live-p ghostel--process))
                (ghostel-send-string
                 (concat (my/ai-term--shell-command p) "\n"))))))
        buf program)
      buf))

  (defun my/ai-term--make-eat (buf-name dir program)
    "Create an eat terminal buffer BUF-NAME in DIR running PROGRAM."
    (let* ((default-directory dir)
           (argv (if (listp program) program (list shell-file-name shell-command-switch program))))
      (apply #'eat-make (string-trim buf-name "*") (car argv) nil (cdr argv))))

  (defun my/ai-term--make-ansi-term (buf-name dir program)
    "Create an ansi-term buffer BUF-NAME in DIR running PROGRAM."
    (let ((default-directory dir))
      (with-current-buffer (ansi-term shell-file-name (string-trim buf-name "*"))
        (term-send-raw-string (concat (my/ai-term--shell-command program) "\n"))
        (current-buffer))))

  (defun my/ai-term--make-buffer (buf-name dir program)
    "Dispatch buffer creation to the configured backend."
    (pcase my/ai-term-backend
  ...
  Reuses an existing live per-project buffer if one exists.
  Kills a stale (dead-process) buffer and starts fresh."
    (let* ((dir (my/ai-term--directory))
           (buf-name (my/ai-term--buffer-name tool dir))
           (existing (get-buffer buf-name)))
      (cond
       ;; Existing live instance — just show it
       ((and existing (my/ai-term--buffer-alive-p existing))
        (my/ai-term--display existing))
       ;; Stale buffer — clean up and restart
       (t
        (when existing (kill-buffer existing))
        (let ((buf (my/ai-term--make-buffer buf-name dir program)))
          (when buf (my/ai-term--display buf))
          buf)))))

  ;;; --- Toggle ---

  (defun my/ai-term-toggle ()
    "Toggle the AI terminal side window for the current project.
  If the window is visible, close it.  If a buffer exists but is hidden,
  show it.  Does nothing if no AI terminal has been started."
    (interactive)
    (let* ((dir (my/ai-term--directory))
           (buf (my/ai-term--find-project-buffer dir)))
      (cond
  ...
  (defun my/ai-term--send (buf string)
    "Send STRING followed by a newline to AI terminal BUF."
    (with-current-buffer buf
      (pcase my/ai-term-backend
        ('ghostel
         (when (and (boundp 'ghostel--process) ghostel--process
        ...
        ('ansi-term
         (term-send-raw-string (concat string "\n"))))))

  (defun my/ai-term-send-command ()
    "Prompt for a string and send it to the current project's AI terminal."
    (interactive)
    (let ((buf (my/ai-term--find-project-buffer (my/ai-term--directory))))
      (if buf
  ...
  (defun my/claude ()
    ...
    (interactive)
    (my/ai-term-launch "claude" (or (executable-find "claude") "claude")))

  (defun my/opencode ()
    "Launch OpenCode in the current project's AI terminal."
    (interactive)
    (my/ai-term-launch "opencode" (or (executable-find "opencode") "opencode")))

  (defun my/droid ()
    "Launch Factory.ai Droid in the current project's AI terminal."
    (interactive)
    (my/ai-term-launch "droid" (or (executable-find "droid") "droid")))

  (defun my/pi ()
    "Launch Pi in the current project's AI terminal."
    (interactive)
    (my/ai-term-launch "pi" (or (executable-find "pi") "pi")))

  (defun my/pi-no-sandbox ()
    "Launch Pi with --no-sandbox in the current project's AI terminal."
    (interactive)
    (my/ai-term-launch "pi-no-sandbox"
                       (list (or (executable-find "pi") "pi") "--no-sandbox")))
